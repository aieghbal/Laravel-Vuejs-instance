<script>
import axios from 'axios'
export default {
    mounted() {
        this.getUserInfo()
    },
    data() {
        return {
            greeting: 'Hello World!'
        }
    },
    methods:{
        getUserInfo(){
            axios.post(`http://localhost:8000/api/users/delete`,data)
                .then(response => {
                    this.closeDelete();
                    this.refreshData();
                })
                .catch(error => {
                    console.log(error);
                });
        }
    }
}
</script>

<template>
    <link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css'><link rel="stylesheet" href="./style.css">
    <div id="nav-bar">
        <input id="nav-toggle" type="checkbox"/>
        <div id="nav-header"><a id="nav-title" href="https://www.linkedin.com/in/am-eghbal/" target="_blank">Ai<i class="fab fa-codepen"></i>eghbal</a>
            <label for="nav-toggle"><span id="nav-toggle-burger"></span></label>
            <hr/>
        </div>
        <div id="nav-content">
            <div class="nav-button"><i class="fas fa-solid fa-database"></i><span>Yajra DataTable</span></div>
            <div class="nav-button"><i class="fab fa-js"></i><span>Vuetify</span></div>
            <div class="nav-button"><i class="fab fa-brands fa-vuejs"></i><span>VueJs DataTable</span></div>
            <hr/>
            <div class="nav-button"><i class="fas fa-heart"></i><span>Following</span></div>
            <div class="nav-button"><i class="fas fa-chart-line"></i><span>Trending</span></div>
            <div class="nav-button"><i class="fas fa-fire"></i><span>Challenges</span></div>
            <div class="nav-button"><i class="fas fa-magic"></i><span>Spark</span></div>
            <hr/>
            <div class="nav-button"><i class="fas fa-gem"></i><span>Codepen Pro</span></div>
            <div id="nav-content-highlight"></div>
        </div>
        <input id="nav-footer-toggle" type="checkbox"/>
        <div id="nav-footer">
            <div id="nav-footer-heading">
                <div id="nav-footer-avatar"><img src="https://gravatar.com/avatar/4474ca42d303761c2901fa819c4f2547"/></div>
                <div id="nav-footer-titlebox">
                    <a id="nav-footer-title" href="https://codepen.io/uahnbu/pens/public" target="_blank">{{ $page.props.auth.user.name }}</a>
                </div>
                <label for="nav-footer-toggle"><i class="fas fa-caret-up"></i></label>
            </div>
            <div id="nav-footer-content">
                <Lorem>
                    web developer. <br>
                    Proficient in PHP and Laravel framework <br>
                    Experience with JavaScript, Vuejs <br>
                </Lorem>
            </div>
        </div>
    </div>
</template>
